...
func (b BlstImplementation) SignatureFromBytes(sig []byte) (bls_interface.Signature, error) {
...
func (b BlstImplementation) NewAggregateSignature() bls_interface.Signature {
	sig := blst.HashToG2([]byte{'m', 'o', 'c', 'k'}, dst).ToAffine()
	return &Signature{s: sig}
}

// AggregateSignatures converts a list of signatures into a single, aggregated sig.
func (b BlstImplementation) AggregateSignatures(sigs []bls_interface.Signature) bls_interface.Signature {
	if len(sigs) == 0 {
		return nil
	}
...
func (b BlstImplementation) Aggregate(sigs []bls_interface.Signature) bls_interface.Signature {
...
func (b BlstImplementation) VerifyMultipleSignatures(sigs [][]byte, msgs [][32]byte, pubKeys []bls_interface.PublicKey) (bool, error) {
