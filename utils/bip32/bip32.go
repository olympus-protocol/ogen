...
import (
	...
	"github.com/olympus-protocol/ogen/utils/base58"
...
func (k *ExtendedKey) pubKeyBytes() ([]byte, error) {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		secret, err := bls.SecretKeyFromBytes(k.key)
		if err != nil {
			return nil, err
		}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if isChildHardened {
		...
		data = append(pubkey, []byte{0, 0, 0, 0}...)
	}
	...
	// the BLS private key.
	ilKey := bls.DeriveSecretKey(ilr[:len(ilr)/2])
	...
	if k.isPrivate {
		...
		parentSecret, err := bls.SecretKeyFromBytes(k.key)
		if err != nil {
			return nil, err
		}

		// childKey = parse256(Il) + parenKey
		ilKey.Add(parentSecret)
		childKeyBytes := ilKey.Marshal()
...
func (k *ExtendedKey) Neuter(net *NetPrefix) (*ExtendedKey, error) {
	...
	return NewExtendedKey(version, pubkey, k.chainCode, k.parentFP,
		k.depth, k.childNum, false), nil
}

// BlsPubKey converts the extended key to a bls public key and returns it.
func (k *ExtendedKey) BlsPubKey() (*bls.PublicKey, error) {
	pubBytes, err := k.pubKeyBytes()
	if err != nil {
		return nil, err
	}
...
func (k *ExtendedKey) BlsPrivKey() (*bls.SecretKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	return bls.SecretKeyFromBytes(k.key)
}
...
func (k *ExtendedKey) String() string {
	...
	if k.isPrivate {
		...
		serializedBytes = append(serializedBytes, pubBytes...) // 48 bytes
	}
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(bls.RFieldModulus) >= 0 || keyNum.Sign() == 0 {
