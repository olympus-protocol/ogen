...
func (s *SerializableState) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if s.CoinsState == nil {
		s.CoinsState = new(primitives.CoinsStateSerializable)
	}
	...
	dst = ssz.WriteOffset(dst, offset)
	if s.Governance == nil {
		s.Governance = new(primitives.GovernanceSerializable)
	}
...
func (s *SerializableState) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'ValidatorRegistry'
	{
		...
		s.ValidatorRegistry = make([]*primitives.Validator, num)
		for ii := 0; ii < num; ii++ {
			if s.ValidatorRegistry[ii] == nil {
				s.ValidatorRegistry[ii] = new(primitives.Validator)
			}
	...
	// Field (16) 'CurrentEpochVotes'
	{
		...
		s.CurrentEpochVotes = make([]*primitives.AcceptedVoteInfo, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if s.CurrentEpochVotes[indx] == nil {
				s.CurrentEpochVotes[indx] = new(primitives.AcceptedVoteInfo)
			}
	...
	// Field (19) 'PreviousEpochVotes'
	{
		...
		s.PreviousEpochVotes = make([]*primitives.AcceptedVoteInfo, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if s.PreviousEpochVotes[indx] == nil {
				s.PreviousEpochVotes[indx] = new(primitives.AcceptedVoteInfo)
			}
