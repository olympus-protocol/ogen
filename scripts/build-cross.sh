...
CC=x86_64-linux-gnu-gcc CXX=x86_64-linux-gnu-g++  CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build cmd/ogen-d/ogen-d.go
...
CC=aarch64-linux-gnu-gcc CXX=aarch64-linux-gnu-g++ CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build cmd/ogen-d/ogen-d.go
...
CXX=x86_64-apple-darwin19-clang++ CC=x86_64-apple-darwin19-clang CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build cmd/ogen-d/ogen-d.go
...
CXX=x86_64-w64-mingw32-c++ CC=x86_64-w64-mingw32-gcc CGO_ENABLED=1 GOOS=windows GOARCH=amd64 go build -tags netgo -ldflags '-extldflags "-static -static-libstdc++"' -a cmd/ogen-d/ogen-d.go

mkdir $FOLDER_NAME
mv ogen.exe ./$FOLDER_NAME
mv ogen-cli.exe ./$FOLDER_NAME
mv migration.exe ./$FOLDER_NAME
mv ogen-d.exe ./$FOLDER_NAME
